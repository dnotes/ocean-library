...
export async function getExcerpts(hits:SearchHit[]):Promise<Excerpt[]> {
  ...
  for (let i=1; i<hits.length; i++) {
    ...
    if (docs?.[hit.slug] && docs[hit.slug] !== 'error') excerpts.push({ ...hit, blocksStart:hit.blk, blocksEnd:hit.blk, blocks: [(document?.createRange().createContextualFragment(docs[hit.slug].blocks[hit.blk]).textContent?.trim() || '')] })
  }
  return excerpts
}
